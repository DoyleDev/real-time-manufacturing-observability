...
def get_branch() -> dict:
    return http("GET", f"{API}/{BRANCH_PATH}").json()
...
def ai_analyze(results: list[SqlResult]) -> str:
    ...
    user_msg = (
        ...
        "3. Identify any test cases listed in the PR and assess whether the executed SQL satisfies them.\n"
        "4. Highlight risks (data loss, locking, perf, indexes, constraints, RLS).\n"
        "5. Surface notable statistics from the table stats provided (row counts pre/post if inferable).\n"
        "6. Give a clear merge recommendation: APPROVE / REQUEST_CHANGES / BLOCK with one-line reason.\n"
    ...
    body = {
        ...
        "max_tokens": 2048,
        "messages": [
            {"role": "system", "content": "You are a senior database reviewer for production Postgres changes."},
